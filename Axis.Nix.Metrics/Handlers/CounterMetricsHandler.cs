...
{
    internal class CounterMetricsHandler<TEventData> : IEventHandler<TEventData>
    {
        private Func<DomainEvent<TEventData>, bool> _metricsPredicate;

        private Action<DomainEvent<TEventData>, IMeasureCounterMetrics> _metricsCollector;

        private IMetrics _metrics;


        public CounterMetricsHandler(
            IMetrics metrics,
        ...
        public IOperation HandleEvent(DomainEvent<TEventData> @event)
            => Operation.Try(() => _metricsCollector.Invoke(@event, _metrics.Measure.Counter));
    }
}
